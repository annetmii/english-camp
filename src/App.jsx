...
/**
 * - onDraft: 打鍵ごと（親stateは触らない）
 * - onCommit: Blur/Enter/IME確定など「確定時のみ」親に反映（setWsはここだけ）
 * これで再レンダー頻度が下がり、iPhoneでキーボードが閉じる/PCで1文字ずつしか動かない問題を解消。
 */
const DebouncedInput = React.memo(function DebouncedInput({
  ...
  rows = 1,
  autoGrow = true,
  commitOnEnter = true, // 単行のみ Enter で確定
  ...rest
}) {
  ...
  const common = {
    ...
    onKeyDown: (e) => {
      if (multiline || !commitOnEnter) return;
      if (e.key === "Enter") {
        e.preventDefault();
        commit(inner);
        try {
          ...
          <button
            ...
            onClick={() =>
              setWs((cur) => ({
                ...
                parts: {
                  ...cur.parts,
                  part1: { ...cur.parts.part1, items: [...cur.parts.part1.items, { id: genId(), en: "" }] },
                },
              }))
            }
          >
      ...
      {mode === "trainer" ? (
        <div style={{ marginTop: 12 }}>
          ...
          <DebouncedInput
            ...
            onDraft={() => {}}
            onCommit={(v) =>
              setWs((cur) => ({ ...cur, parts: { ...cur.parts, part1: { ...cur.parts.part1, trainerNotes: v } } }))
            }
          />
        </div>
  ...
  return (
    <Card title={ws.parts.part4.label} instructions={ws.parts.part4.instructions}>
      <DebouncedInput
        ...
        onDraft={() => {}}
        onCommit={(v) =>
          setWs((cur) => ({ ...cur, parts: { ...cur.parts, part4: { ...cur.parts.part4, answer: v } } }))
        }
      />
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <DebouncedInput
            ...
            onCommit={(v) =>
              setWs((cur) => ({
                ...cur,
                parts: { ...cur.parts, part4: { ...cur.parts.part4, trainerNotes: v } },
              }))
            }
          />
        </div>
  ...
  return (
    <div className="card" style={{ padding: "12px", background: "white" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
        <button className="btn" onClick={() => onSelect(todayISO(new Date(y, m - 1, 1)))}>
          ◀
        </button>
      ...
      <div className="header-row">
        <div className="container">
          <div className="hstack">
            <button className="hdr-btn" onClick={() => setShowCal((v) => !v)}>
              カレンダー
            </button>
            {mode === "student" ? (
              <>
                <input
                  ...
                  aria-label="講師PIN"
                />
                <button className="hdr-btn-primary" onClick={() => switchToTrainer()}>
                  講師モード
                </button>
              </>
            ) : (
              <button className="hdr-btn" onClick={() => switchToStudent()}>
                学習者モードへ
              </button>
            )}
          </div>
      ...
      <div className="header-row">
        <div className="container">
          <div className="hstack" style={{ justifyContent: "space-between" }}>
            <div className="hstack">
              <button className="hdr-btn-primary" onClick={() => doSync("手動同期")}>
                同期
              </button>
              {mode === "trainer" && (
                <button className="hdr-btn danger-btn" onClick={() => resetQuestions()} title="この日の出題を初期化">
                  出題リセット
                </button>
              )}
              <button className="hdr-btn submit-btn" onClick={() => submit()}>
                提出
              </button>
            </div>
...
const defaultWorksheet = (dateISO) => ({
  ...
  parts: {
    ...
    part4: {
      label: "Part 4｜英作文",
      instructions: "本日のテーマに沿って80–120語で英作文を作ろう。",
      answer: "",
      handwriting: null,
      trainerNotes: "",
    },
...
export default function App() {
    ...
    (async () => {
      try {
        ...
        if (remote && remote.data) {
          setWs((_) => ensureMaps(remote.data, dateISO));
          setStatus("クラウドから読み込みました");
        } else {
          setWs((_) => ensureMaps(defaultWorksheet(dateISO), dateISO));
  ...
  const switchToTrainer = useCallback(() => {
    if (pinInput === DEFAULT_PIN) setMode("trainer");
    else alert("PINが違います。");
  }, [pinInput]);
  const switchToStudent = useCallback(() => setMode("student"), []);

  /* ===== Fetch & merge loop (other devices) ===== */
  const fetchAndMerge = useCallback(async () => {
    try {
      const remote = await cloudLoad({ userId, dateISO });
      if (remote && remote.data) {
        setWs((cur) => ensureMaps(remote.data, dateISO)); // 競合が気になるなら diff マージに戻せます
      }
    } catch {}
  }, [userId, dateISO]);
  useEffect(() => {
    ...
    <Card title="本日のテーマ">
      {mode === "trainer" ? (
        <DebouncedInput
          ...
          rows={2}
          autoGrow
          className="input field-full theme-input"
        ...
        <p className="theme-text" style={{ margin: 0, whiteSpace: "pre-line" }}>{ws.meta.theme || "（未設定）"}</p>
      )}
    </Card>
  ));
  ...
  const resetQuestions = useCallback(() => {
    ...
    setWs((cur) => {
      const next = defaultWorksheet(cur.meta.date);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(to bottom, #fff, #f9fafb)" }}>
      <Header
        ...
        switchToTrainer={() => switchToTrainer()}
