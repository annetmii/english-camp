/* app.jsx — 完全版（CDN React + Netlify Functions / ビルド不要）
 * 要件：
 * - 学習者は「開く→解答→送信」だけ
  ...
  return (
    <div className="container">
      ...
      <div className="sticky">
        <h1 className="title">annetmii English Camp</h1>
      ...
      <div className="card">
        ...
        <div className="muted" style={{ marginTop: 6 }}>
          ・クラウド同期は「送信」時と入力が止まって約60秒後に自動実行。起動/切替で自動読み込み。
        </div>
      </div>
          ...
          <div key={q.id} className="card">
            {role === "trainer" ? (
              ...
              <Row>
                ...
                <div className="muted">ヒント：{q.hint || "—"}</div>
                <div>
                ...
                <div className="muted">参考：正解の日本語訳（講師側入力）→ {q.ja || "（未設定）"}</div>
              </Row>
            )}
          </div>
        ))}
      ...
      <section style={{ marginTop: 8 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 12 }}>
            {role === "student" ? (
              <button onClick={submit} className="btn">解答を送信（クラウド保存）</button>
            ) : (
              <button
                onClick={async () => {
                  try {
      ...
      <section style={{ marginTop: 8, marginBottom: 48 }}>
        ...
        <div className="muted" style={{textAlign:"right"}}>{syncStatus}</div>
      </section>
    </div>
  );
}
